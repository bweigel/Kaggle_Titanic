...
multinom0 <- function(formula, data, k, ...){
  ...
  for(i in 1:k){
    ...
    if(!exists("sets")) sets <- list(list(validation = tmp_samples, 
                                     model = multinom(formula=formula, data = data, ...)))
    else sets <- append(sets, list(list(validation = tmp_samples,
                                   model = multinom(formula=formula, data = data, ...))))
  ...
  outPut <- function(...){
    if(length(list(...)) == 0) return(list(overallErr = NA, Err = NA, models = sets))
    else {
      
      return(list(overallErr = mean(errors), Err = errors, models = sets))
    }
  }
  return(outPut)
}
